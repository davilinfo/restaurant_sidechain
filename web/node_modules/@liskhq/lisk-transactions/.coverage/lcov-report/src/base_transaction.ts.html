...
<head>
    <title>Code coverage report for src/base_transaction.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> base_transaction.ts</h1>
        <div class='clearfix'>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>94/110</span>
            </div>
...
&nbsp;
	public toJSON(): TransactionJSON {
		const transaction = {
			...
			senderPublicKey: this._senderPublicKey || <span class="branch-1 cbranch-no" title="branch not covered" >'',</span>
			senderId: this._senderPublicKey ? this.senderId : <span class="branch-1 cbranch-no" title="branch not covered" >'',</span>
...
&nbsp;
		...
		<span class="missing-if-branch" title="else path not taken" >E</span>if (multiSigError) {
...
&nbsp;
		...
		const updatedSender = {
			...
			publicKey: sender.publicKey || <span class="branch-1 cbranch-no" title="branch not covered" >this.senderPublicKey,</span>
		};
...
<span class="branch-1 cbranch-no" title="branch not covered" >			errors.length === 1 </span>&amp;&amp;
<span class="branch-2 cbranch-no" title="branch not covered" >			errors[0] instanceof TransactionPendingError</span>
		) {
...
&nbsp;
	public <span class="fstat-no" title="function not covered" >async </span>prepare(store: StateStorePrepare): Promise&lt;void&gt; {
<span class="cstat-no" title="statement not covered" >		await store.account.cache([</span>
			{
...
&nbsp;
		...
		<span class="missing-if-branch" title="else path not taken" >E</span>if (valid) {
...
&nbsp;
<span class="cstat-no" title="statement not covered" >		return createResponse(this.id, errors);</span>
	}
&nbsp;
...
&nbsp;
	public isExpired(date: Date = new Date()): boolean {
		<span class="missing-if-branch" title="if path not taken" >I</span>if (!this.receivedAt) {
<span class="cstat-no" title="statement not covered" >			this.receivedAt = new Date();</span>
		}
		...
		const timeOut =
			this._multisignatureStatus === MultisignatureStatus.PENDING ||
			this._multisignatureStatus === MultisignatureStatus.READY
				? <span class="branch-0 cbranch-no" title="branch not covered" >UNCONFIRMED_MULTISIG_TRANSACTION_TIMEOUT</span>
				: UNCONFIRMED_TRANSACTION_TIMEOUT;
		const timeElapsed =
			// tslint:disable-next-line no-magic-numbers
...
&nbsp;
		<span class="missing-if-branch" title="if path not taken" >I</span>if (this._senderPublicKey !== '' &amp;&amp; this._senderPublicKey !== publicKey) {
<span class="cstat-no" title="statement not covered" >			throw new Error(</span>
...
&nbsp;
		if (
			...
			<span class="missing-if-branch" title="if path not taken" >I</span>if (senderIdError) {
<span class="cstat-no" title="statement not covered" >				errors.push(senderIdError);</span>
			}
		}
&nbsp;
		return errors;
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Fri Feb 07 2020 13:23:04 GMT+0100 (Central European Standard Time)
            </div>
        </div>
