...
class PeerPool extends events_1.EventEmitter {
    constructor(peerPoolConfig) {
        ...
        this._handleBanPeer = (peerId) => {
            const unbanTimeout = setTimeout(this._handleUnbanPeer.bind(this, peerId), this._peerPoolConfig.peerBanTime);
    ...
    async request(packet) {
        ...
        const peerInfoForRequest = outboundPeerInfos.length === 0
        ...
        const listOfPeerInfo = [
            ...
            internalState: {
                ...
                advertiseAddress: peer.peerInfo.internalState
                    ...
                    : true,
                connectionKind: peer instanceof peer_1.OutboundPeer
                    ? constants_1.ConnectionKind.OUTBOUND
                    : constants_1.ConnectionKind.INBOUND,
            },
        }));
    ...
    discoverFromSeedPeers() {
        ...
        const seedPeersForDiscovery = shuffle(this._peerLists.seedPeers.slice(0, freeOutboundSlots));
    ...
    triggerNewConnections(newPeers, triedPeers) {
        ...
        const disconnectedFixedPeers = this._peerLists.fixedPeers.filter(peer => !this._peerMap.has(peer.peerId));
        ...
        ].forEach((peerInfo) => this._addOutboundPeer(peerInfo, this._nodeInfo));
    }
    ...
    _addOutboundPeer(peerInfo, nodeInfo) {
        ...
        const outboundConnectedPeer = this.getPeers(peer_1.OutboundPeer).find(p => p.ipAddress === peerInfo.ipAddress &&
            p.ipAddress !== constants_1.DEFAULT_LOCALHOST_IP);
    ...
    getFreeOutboundSlots() {
        ...
        const disconnectedFixedPeers = this._peerLists.fixedPeers.filter(peer => !this._peerMap.has(peer.peerId));
        const openOutboundSlots = this._maxOutboundConnections -
    ...
    _disconnectFromSeedPeers() {
        ...
        outboundPeers.forEach((outboundPeer) => {
            const isFixedPeer = this._peerLists.fixedPeers.find((peer) => peer.peerId === outboundPeer.id);
            const isSeedPeer = this._peerLists.seedPeers.find((peer) => peer.peerId === outboundPeer.id);
    ...
    _bindHandlersToPeer(peer) {
        ...
        peer.on(events_2.EVENT_UNBAN_PEER, this._handleUnbanPeer);
    ...
    _unbindHandlersFromPeer(peer) {
        ...
        peer.removeListener(events_2.EVENT_UNBAN_PEER, this._handleUnbanPeer);
